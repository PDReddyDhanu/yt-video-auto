...
export default function StudioPage({ initialPlatform = 'youtube' }: { initialPlatform?: 'youtube' | 'instagram' }) {
  ...
  const [scriptDuration, setScriptDuration] = useState<number>(120);
  ...
  const [isLockedAvatar, setIsLockedAvatar] = useState<boolean>(false);
  ...
  const [avatarVideoUrl, setAvatarVideoUrl] = useState<string>(`${BACKEND_URL}/uploads/default_avatar.webm`);
    const [selectedAvatarPreset, setSelectedAvatarPreset] = useState<'white' | 'black' | 'custom'>('white');
  const [registeredAvatars, setRegisteredAvatars] = useState<Array<{ id: string; name: string; url: string; preset: string; isDefault?: boolean }>>([
    { id: 'white', name: 'White Avatar', url: `${BACKEND_URL}/uploads/default_avatar.webm`, preset: 'white', isDefault: true },
    { id: 'black', name: 'Black Avatar', url: `${BACKEND_URL}/uploads/black_avatar.webm`, preset: 'black', isDefault: true }
  ]);

  // Chroma Key Background Remover Modal State
  const [showChromaModal, setShowChromaModal] = useState<boolean>(false);
  const [chromaVideoFile, setChromaVideoFile] = useState<File | null>(null);
  const [chromaKeyColor, setChromaKeyColor] = useState<string>('#00FF00');
  const [chromaSimilarity, setChromaSimilarity] = useState<number>(0.38);
  const [chromaBlend, setChromaBlend] = useState<number>(0.08);
  const [removeWhiteRing, setRemoveWhiteRing] = useState<boolean>(true);
  const [isProcessingChroma, setIsProcessingChroma] = useState<boolean>(false);
  const [chromaError, setChromaError] = useState<string>('');
  ...
  const [avatarAspectRatio, setAvatarAspectRatio] = useState<number>(1080 / 1920);
  ...
  const handlePdrAutoWorkflow = async (overrideFile?: File) => {
    ...
    try {
      ...
      // Step 5: Generate Audio & Auto-Fill Captions (Runs TWICE automatically for 100% accuracy)
      setPdrAutoStatus(`🔊 Step 4/4 (Run 1/2): Generating Audio & Auto-Filling Captions...`);
      ...
      // Step 6: Second TTS Generation Run (Mandatory 2nd pass for perfect alignment)
      setPdrAutoStatus(`🔊 Step 4/4 (Run 2/2): Finalizing Word Timestamps & Captions Alignment...`);
      await new Promise(resolve => setTimeout(resolve, 600));

      const ttsRes2 = await fetch(`${BACKEND_URL}/backend/tts/generate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text: ttsPayloadText,
      ...
      if (ttsRes2.ok) {
        ...
        const audioFetchRes2 = await fetch(`${BACKEND_URL}${ttsData2.url}`);
        const audioBlob2 = await audioFetchRes2.blob();
        const audioFileObj2 = new File([audioBlob2], ttsData2.filename, { type: 'audio/mpeg' });
        const audioObjUrl2 = URL.createObjectURL(audioBlob2);

        setAudioFile(audioFileObj2);
        setAudioUrl(audioObjUrl2);

        const words2: WordTimestamp[] = ttsData2.word_timestamps || [];
        let finalAlignedWords: WordTimestamp[] = [];

        if (words2.length > 0 && captionWords.length > 0) {
          if (words2.length === captionWords.length) {
            finalAlignedWords = captionWords.map((word: string, idx: number) => ({
              word,
              start: words2[idx].start,
              end: words2[idx].end
            }));
          } else {
            const numWords = captionWords.length;
            const totalDuration = (words2[words2.length - 1] && words2[words2.length - 1].end) ? words2[words2.length - 1].end : detectedAudioDur;
            const wordDur = totalDuration / numWords;
            finalAlignedWords = captionWords.map((word: string, idx: number) => ({
              word,
              start: parseFloat((idx * wordDur).toFixed(2)),
          ...
          const numWords = captionWords.length;
          const wordDur = (detectedAudioDur || targetDuration) / numWords;
          finalAlignedWords = captionWords.map((word: string, idx: number) => ({
            word,
            start: parseFloat((idx * wordDur).toFixed(2)),
            end: parseFloat(((idx + 1) * wordDur).toFixed(2))
        ...
        setWordTimestamps(finalAlignedWords);

        // Always build 4-word segment captions for Step 6 & FFmpeg rendering
        if (finalAlignedWords.length > 0) {
          const groupSize = 4;
          const finalSegments: CaptionSegment[] = [];
          for (let i = 0; i < finalAlignedWords.length; i += groupSize) {
            const group = finalAlignedWords.slice(i, i + groupSize);
            finalSegments.push({
              id: `auto-${Date.now()}-${i}`,
              start: group[0].start,
              end: group[group.length - 1].end,
          ...
          setCaptions(finalSegments);
        }
      }

      setPdrAutoStatus(`✅ PDR-Auto Complete! 2 TTS Passes Done. Audio & Captions Ready. Destination: READY_TO_APPROVE (1HKOpwOro1jxv09xBqbnhilhtCRb7V8yA).`);
    } catch (err: any) {
  ...
  const TTS_FIXED_SPEED = 1.40;

  // TTS (Text-to-Speech) State
  const [postInfoText, setPostInfoText] = useState<string>('');  // Raw info input for PDR script generation
  const [ttsScript, setTtsScript] = useState<string>('');         // Generated Telugu script (audio source)
  const [tenglishScript, setTenglishScript] = useState<string>(''); // Generated Tenglish script
  const [selectedPresetId, setSelectedPresetId] = useState<string>('MV1');
  ...
  const fetchAvatars = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/backend/avatars`);
      if (res.ok) {
        const data = await res.json();
        if (Array.isArray(data) && data.length > 0) {
          const formatted = data.map((av: any) => ({
            ...av,
            url: av.url.startsWith('http') ? av.url : `${BACKEND_URL}${av.url}`
          }));
          setRegisteredAvatars(formatted);
        }
      }
    } catch (e) {
      console.warn("Could not fetch registered avatars:", e);
    }
  };
  ...
  const handleGetAIScript = async () => {
    if (!postInfoText.trim() && !imageFile) {
      setScriptError('Please paste raw text in the information text box OR upload an image!');
      return;
    }
    ...
    setTtsScript('');
    setTenglishScript('');
    setRecommendedVoice('Male');

    try {
      let res: Response;

      if (imageFile && !postInfoText.trim()) {
        // Image only: use FormData for file upload (OCR mode)
        const formData = new FormData();
        formData.append('image', imageFile);
        formData.append('duration', scriptDuration.toString());
        res = await fetch(`${BACKEND_URL}/backend/generate-script-pdr`, {
          method: 'POST',
          body: formData
        });
      } else {
        // Text only OR text + image: send as JSON (most reliable, no multer issues)
        res = await fetch(`${BACKEND_URL}/backend/generate-script-pdr`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            text: postInfoText.trim(),
      ...
      if (!res.ok) {
        let errMsg = 'Failed to generate PDR scripts.';
      ...
      const teluguOut = data.teluguScript || data.teluguScript || '';
      const tenglishOut = data.tenglishScript || data.tenglishScript || '';
      setTtsScript(teluguOut);
      setTenglishScript(tenglishOut);

      // Automatically set the custom filename if generated by AI
      if (data.autoFilename) {
        setCustomFilename(data.autoFilename);
      } else {
        const fallbackText = tenglishOut || 'rendered_video';
      ...
      // Voice recommendation and auto-selection (always Male for PDR)
      setRecommendedVoice('Male');
      const matchingPreset = TTS_PRESETS.find(p => p.gender === 'M');
      if (matchingPreset) {
      ...
      console.error('[PDR Script Gen Error]', e);
  ...
  // Unified Dragging Logic (Supports wm1, wm2, and avatar)
  const handleMovableMouseDown = (e: React.MouseEvent<HTMLDivElement>, target: 'wm1' | 'wm2' | 'avatar') => {
    e.stopPropagation();
    setActiveWatermark(target);
    const locked = target === 'wm1' ? isLockedWm1 : (target === 'wm2' ? isLockedWm2 : isLockedAvatar);
    if (locked) return;
    ...
    if (containerRef.current) {
      ...
      const currentY = target === 'wm1' ? movableY : (target === 'wm2' ? movable2Y : avatarY);
      dragStart.current = {
        x: e.clientX,
        y: e.clientY,
  ...
  const handleMovableTouchStart = (e: React.TouchEvent<HTMLDivElement>, target: 'wm1' | 'wm2' | 'avatar') => {
    e.stopPropagation();
    setActiveWatermark(target);
    const locked = target === 'wm1' ? isLockedWm1 : (target === 'wm2' ? isLockedWm2 : isLockedAvatar);
    if (locked) return;
  ...
  // Dynamic layout calculations for Avatar character cropping & aspect ratio
  const scaleAvatarX = 1 - (cropAvatarLeft + cropAvatarRight) / 100;
  const scaleAvatarY = 1 - (cropAvatarTop + cropAvatarBottom) / 100;
  const croppedAvatarAspectRatio = avatarAspectRatio * (scaleAvatarX / Math.max(0.01, scaleAvatarY));
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDragging) {
        ...
        if (dragTarget.current === 'wm1') {
          ...
          setMovableX(newX);
          setMovableY(newY);
        } else if (dragTarget.current === 'wm2') {
          newX = Math.max(0, Math.min(100 - movable2Scale, newX));
          newY = Math.max(0, Math.min(100 - movable2HeightPct, newY));
          setMovable2X(newX);
          setMovable2Y(newY);
        } else if (dragTarget.current === 'avatar') {
          const avatarHeightPct = (1080 / 1920) * avatarScale / (croppedAvatarAspectRatio || (9/16));
          newX = Math.max(0, Math.min(100 - avatarScale, newX));
          newY = Math.max(0, Math.min(100 - avatarHeightPct, newY));
          setAvatarX(newX);
          setAvatarY(newY);
        }
      } else if (isResizing) {
        ...
        if (isResizing === 'br' || isResizing === 'tr') {
          ...
          if (dragTarget.current === 'wm1') {
            ...
            setAvatarX(Math.max(0, Math.min(100 - avatarScale, newX)));
          }
        }
  ...
  const handleProcessChromaKey = async () => {
    if (!chromaVideoFile) {
      setChromaError('Please select a video file first.');
      return;
    }
    setChromaError('');
    ...
    try {
      const formData = new FormData();
      formData.append('avatarVideo', chromaVideoFile);
      formData.append('keyColor', chromaKeyColor);
      formData.append('similarity', chromaSimilarity.toString());
      formData.append('blend', chromaBlend.toString());
      formData.append('removeWhiteRing', removeWhiteRing.toString());
      formData.append('avatarName', chromaVideoFile.name.replace(/\.[^/.]+$/, ''));

      const res = await fetch(`${BACKEND_URL}/backend/avatars/remove-background`, {
        method: 'POST',
        body: formData
      });
      ...
      const data = await res.json();
      const newAvatarUrl = `${BACKEND_URL}${data.avatarUrl}`;
      
      setAvatarVideoUrl(newAvatarUrl);
      setSelectedAvatarPreset('custom');
      setShowChromaModal(false);
      setChromaVideoFile(null);
      await fetchAvatars();
    } catch (err: any) {
      console.error("Chroma key error:", err);
      setChromaError(err.message || 'Chroma Key background removal failed.');
    } finally {
  ...
  const handleGenerateVideo = async () => {
    ...
    formData.append('avatarPreset', selectedAvatarPreset);
    if (avatarVideoUrl) {
      formData.append('avatarUrl', avatarVideoUrl);
    }
    if (avatarVideoFile) {
    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
        ...
        <div className="space-y-6 lg:col-span-7 order-2 lg:order-1">
            ...
            {backgrounds.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                {backgrounds.filter(bg => {
                    const fn = (bg.filename || bg.name || '').toLowerCase();
                    return !fn.includes('avatar') && !fn.includes('flexclip') && !fn.includes('black_background') && !fn.includes('i_want_the_black');
                  }).map((bg) => (
                  <button
                    key={bg.id}
                    onClick={() => {
          ...
          {/* PDR STEP 2: Post Information Text Area & Dual Script Generator (Telugu + Tenglish) */}
          <div className="rounded-2xl border border-amber-500/40 bg-slate-900/60 p-6 backdrop-blur-md space-y-4 shadow-xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/10 text-amber-400 font-bold text-sm border border-amber-500/20">
                  2
                </div>
                <div>
                  <h2 className="text-base font-bold text-amber-300 uppercase tracking-wider flex items-center gap-2">
                    <FileText className="h-5 w-5 text-amber-400" /> Raw News / Scheme Info & Dual Script Generator
                  </h2>
                  <p className="text-xs text-slate-400">Paste raw text or upload image to generate matched Telugu & Tenglish audio scripts (120-150s).</p>
                </div>
              </div>
              <span className="px-2.5 py-1 text-[10px] font-black bg-amber-500 text-slate-950 rounded-lg uppercase tracking-wider">PDR Mode</span>
            </div>

            {/* Unlimited Raw Text Input Box */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-200 flex items-center justify-between">
                <span>Unlimited Raw Information Text Box (No Character Limit)</span>
                <span className="text-[10px] text-slate-400 font-mono">{postInfoText.length} characters</span>
              </label>
              <textarea
                rows={5}
                placeholder="Paste raw Telugu text / petrol-diesel prices / scheme details here (e.g., ⛽ భారతదేశం | పెట్రోల్, డీజిల్ ధరలు ఎలా నిర్ణయిస్తారు? 📌 ముఖ్యాంశాలు: దేశంలో పెట్రోల్, డీజిల్ ధరలు...)"
                value={postInfoText}
                onChange={(e) => setPostInfoText(e.target.value)}
                className="w-full p-3.5 rounded-xl border border-amber-500/30 bg-slate-950 text-xs text-slate-100 placeholder-slate-500 focus:border-amber-400 focus:outline-none resize-none leading-relaxed font-sans"
              />
            </div>

            {/* OCR Image Dropzone & Duration Controls */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-center pt-2">
              <label className="flex items-center gap-2.5 p-3 rounded-xl border border-dashed border-slate-700 bg-slate-950/60 hover:border-amber-500/50 cursor-pointer transition-all">
                <Upload className="h-5 w-5 text-amber-400 shrink-0" />
                <div className="text-left">
                  <p className="text-xs font-semibold text-slate-200">
                    {imageFile ? imageFile.name : "Or Upload Graphic Image (Auto OCR)"}
                  </p>
                  <p className="text-[10px] text-slate-500">Extracts text if text box is empty</p>
                </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1">
                  ...
                  <select
                    ...
                    className="w-full p-2.5 rounded-xl border border-slate-700 bg-slate-950 text-xs text-slate-200 focus:border-amber-400 focus:outline-none cursor-pointer"
                  >
                    <option value={120}>120 Seconds (PDR Default)</option>
                    <option value={125}>125 Seconds (Reels)</option>
                    <option value={140}>140 Seconds (Optimal PDR)</option>
                    <option value={150}>150 Seconds (Shorts Max)</option>
                  </select>
                </div>
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-black text-xs flex items-center gap-1.5 shadow-lg shadow-amber-500/20 cursor-pointer transition-all disabled:opacity-50"
                >
                  {isGeneratingScript ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
                  Generate 125-150s PDR Scripts
                </button>
              </div>
            </div>

            {/* Generated Dual Audio Scripts Boxes */}
            <div className="space-y-4 pt-4 border-t border-slate-800">
              {/* Box 1: TELUGU SCRIPT */}
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-amber-300 flex items-center gap-1.5">
                    <FileText className="h-3.5 w-3.5 text-amber-400" /> TELUGU SCRIPT (తెలుగు) — Human Breathing Pauses (...)
                  </span>
                  <button
                    type="button"
                    onClick={() => navigator.clipboard.writeText(ttsScript)}
                    className="text-[10px] text-slate-400 hover:text-white flex items-center gap-1 bg-slate-800 px-2 py-0.5 rounded"
                  >
                    <Copy className="h-3 w-3" /> Copy Telugu
                  </button>
                </div>
                <textarea
                  rows={4}
                  ...
                  placeholder="Generated Telugu script will appear here with natural human pauses..."
                  className="w-full p-3 rounded-xl border border-slate-800 bg-slate-950 text-xs text-slate-100 font-sans leading-relaxed focus:border-amber-400 focus:outline-none"
                />
              </div>

              {/* Box 2: TENGLISH SCRIPT */}
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-orange-300 flex items-center gap-1.5">
                    <FileText className="h-3.5 w-3.5 text-orange-400" /> TENGLISH SCRIPT (English Alphabet) — 100% Matched Line-by-Line
                  </span>
                  <button
                    type="button"
                    onClick={() => navigator.clipboard.writeText(tenglishScript)}
                    className="text-[10px] text-slate-400 hover:text-white flex items-center gap-1 bg-slate-800 px-2 py-0.5 rounded"
                  >
                    <Copy className="h-3 w-3" /> Copy Tenglish
                  </button>
                </div>
                <textarea
                  rows={4}
                  ...
                  placeholder="Generated Tenglish script will appear here (100% matched word-for-word)..."
                  className="w-full p-3 rounded-xl border border-slate-800 bg-slate-950 text-xs text-slate-100 font-sans leading-relaxed focus:border-amber-400 focus:outline-none"
                />
              </div>

              {/* Call-to-Action Indicator */}
              <div className="p-2.5 rounded-xl border border-emerald-500/30 bg-emerald-950/30 flex items-center justify-between text-xs text-emerald-300">
                <span className="font-bold flex items-center gap-1.5">
                  <CheckCircle2 className="h-4 w-4 text-emerald-400" /> Matched CTA Included:
                </span>
                <span className="font-semibold text-[11px] font-sans">
                  "ఇప్పుడే Telugu States Official ను ఫాలో అవ్వండి!" / "Ippude Telugu States Official ni Follow Avvandi!"
                </span>
              </div>
            </div>
          ...
          {/* Step 3: Center Image Overlay (Moved smoothly to Step 3) */}
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur-sm">
            <div className="mb-4 flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-orange-500/10 text-orange-400 font-semibold text-sm border border-orange-500/20">
                3
              </div>
              <h2 className="text-lg font-semibold text-slate-200">Upload Center Graphic Image</h2>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-orange-500/20 bg-indigo-950/10 p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-950/60 border border-slate-800/60">
                  ...
                  <span className="text-[10px] font-mono text-orange-300 ml-2 font-bold">1.40x Speed</span>
                </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur-sm">
            <div className="mb-4 flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-slate-200">Avatar Character Layer & Direct Crop</h2>
                  <p className="text-[10px] text-slate-400">Position speaker avatar, lock movement, and crop avatar video directly before rendering.</p>
                </div>
              </div>

              <div className="flex items-center gap-2">
                {/* Lock Avatar Movement Switch */}
                <button
                  type="button"
                  onClick={() => setIsLockedAvatar(!isLockedAvatar)}
                  ...
                  title={isLockedAvatar ? "Avatar position is LOCKED. Click to unlock dragging." : "Avatar position is UNLOCKED. Drag freely on simulator canvas."}
                >
                  {isLockedAvatar ? (
                    <><Lock className="h-3.5 w-3.5" /> Locked</>
                  ) : (
                    <><Unlock className="h-3.5 w-3.5" /> Moveable</>
                  )}
                </button>
            ...
            {showAvatar && (
              <div className="space-y-4 pt-2">
                {/* Avatar Characters Module & Background Remover Button */}
                <div className="p-4 rounded-xl border border-slate-800 bg-slate-950/60 space-y-3">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div>
                      <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wide">Avatar Characters Module</h3>
                      <p className="text-[10px] text-slate-400">Select processed transparent avatars or remove background from new videos.</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setShowChromaModal(true)}
                      className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-md border border-emerald-400/40"
                    >
                      <Sparkles className="h-4 w-4 text-yellow-200 animate-pulse" />
                      Open Background Remover (Chroma Key)
                    </button>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1">
                    {registeredAvatars.map((av) => (
                      <button
                        key={av.id}
                        type="button"
                        onClick={() => {
                          setSelectedAvatarPreset(av.preset as any);
                          setAvatarVideoUrl(av.url);
                          setAvatarVideoFile(null);
                        }}
                        className={`p-3 rounded-xl border flex flex-col items-center gap-1.5 transition-all cursor-pointer text-left ${
                          avatarVideoUrl === av.url
                            ? 'border-orange-500 bg-slate-900 shadow-md ring-1 ring-orange-500'
                            : 'border-slate-800 bg-slate-950/80 hover:bg-slate-900 text-slate-400'
                        }`}
                      >
                        <div className="flex items-center gap-1.5">
                          <span className={`w-2.5 h-2.5 rounded-full border ${av.preset === 'white' ? 'bg-white border-slate-400' : 'bg-slate-950 border-emerald-400'}`} />
                          <span className="text-xs font-bold text-slate-200 truncate max-w-[110px]">{av.name}</span>
                        </div>
                        <span className="text-[9px] text-slate-400">{av.isDefault ? 'Standard Character' : 'Transparent WebM'}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {/* Crop Avatar Video Margins */}
                <div className="p-4 rounded-xl border border-slate-800 bg-slate-950/40 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-300">Direct Avatar Video Cropping</span>
                    <div className="flex items-center gap-1.5">
                      <button
                        type="button"
                        onClick={() => { setCropAvatarTop(0); setCropAvatarBottom(0); setCropAvatarLeft(0); setCropAvatarRight(0); }}
                        className="px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-[9px] font-semibold transition-colors"
                      ...
                      <button
                        type="button"
                        onClick={() => { setCropAvatarTop(0); setCropAvatarBottom(35); setCropAvatarLeft(0); setCropAvatarRight(0); }}
                        className="px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-orange-300 text-[9px] font-semibold transition-colors"
                        title="Crop bottom 35% (Upper Body Focus)"
                      >
                        Upper Body
                      </button>
                      <button
                        type="button"
                        onClick={() => { setCropAvatarTop(10); setCropAvatarBottom(50); setCropAvatarLeft(10); setCropAvatarRight(10); }}
                        className="px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-amber-300 text-[9px] font-semibold transition-colors"
                        title="Face & Headshot Focus"
                      >
                        Headshot
                      </button>
                    </div>
                    <button
                      ...
                      onClick={() => {
                        setCropAvatarTop(0);
                        setCropAvatarBottom(0);
                        setCropAvatarLeft(0);
                        setCropAvatarRight(0);
                      }}
                      className="text-[10px] text-slate-400 hover:text-orange-400 underline font-medium transition-colors"
                    >
                      Reset Crop
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        max="80"
                        value={cropAvatarTop}
                        onChange={(e) => setCropAvatarTop(parseInt(e.target.value))}
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] text-slate-400">
                        <span>Crop Bottom</span>
                        <span>{cropAvatarBottom}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                        max="80"
                        value={cropAvatarBottom}
                        onChange={(e) => setCropAvatarBottom(parseInt(e.target.value))}
                        className="w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-orange-500"
                      />
                    </div>
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] text-slate-400">
                        <span>Crop Left</span>
                        <span>{cropAvatarLeft}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                        max="80"
                        value={cropAvatarLeft}
                        onChange={(e) => setCropAvatarLeft(parseInt(e.target.value))}
                        className="w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-orange-500"
                      />
                    </div>
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] text-slate-400">
                        <span>Crop Right</span>
                        <span>{cropAvatarRight}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                        max="80"
                        value={cropAvatarRight}
                        onChange={(e) => setCropAvatarRight(parseInt(e.target.value))}
                        className="w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-orange-500"
                      />
                    </div>
                  </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur-sm flex flex-col items-center">
            ...
            <div className="w-full flex flex-wrap items-center justify-between gap-2 mb-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-1 bg-slate-950 px-2 py-1 rounded-xl border border-slate-800 shadow-sm overflow-x-auto max-w-[280px] sm:max-w-none">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider pr-1 shrink-0">AVATAR:</span>
                  {registeredAvatars.map((av) => (
                    <button
                      key={av.id}
                      type="button"
                      ...
                      className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase flex items-center gap-1 transition-all cursor-pointer shrink-0 ${
                        avatarVideoUrl === av.url
                          ? 'bg-slate-200 text-slate-950 shadow border border-white font-bold'
                          : 'bg-slate-900 text-slate-400 hover:text-slate-200 border border-slate-800'
                      }`}
                    >
                      <span className={`w-1.5 h-1.5 rounded-full border inline-block ${av.preset === 'white' ? 'bg-white border-slate-400' : 'bg-slate-950 border-emerald-400'}`} />
                      {av.name.replace(' Avatar', '')}
                    </button>
                  ))}
                </div>

                {/* PDR-Auto Toggle Switch */}
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-950 border border-slate-800 shadow-sm">
                  <span className="text-[10px] font-bold text-slate-300 uppercase tracking-wider">PDR-Auto</span>
                  <button
                    type="button"
                    onClick={() => setIsPdrAutoMode(!isPdrAutoMode)}
                    className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${isPdrAutoMode ? 'bg-emerald-500' : 'bg-slate-700'}`}
                    title={isPdrAutoMode ? "PDR-Auto Mode ON: Auto-runs pipeline on image paste/upload" : "PDR-Auto Mode OFF: Manual step-by-step mode"}
                  >
                    <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${isPdrAutoMode ? 'translate-x-4' : 'translate-x-0'}`} />
                  </button>
                  <span className={`text-[9px] font-black uppercase px-1 rounded ${isPdrAutoMode ? 'text-emerald-400 bg-emerald-950/60' : 'text-slate-500'}`}>
                    {isPdrAutoMode ? 'ON' : 'OFF'}
                  </span>
                </div>
                ...
                <button
                  type="button"
                  onClick={() => handlePdrAutoWorkflow()}
                  disabled={isPdrAutoRunning}
                  className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-orange-600 via-amber-500 to-orange-500 hover:from-orange-500 hover:to-amber-400 disabled:opacity-50 text-white font-black text-xs shadow-md shadow-orange-950/40 border border-orange-400/40 flex items-center gap-1.5 transition-all hover:scale-105 active:scale-95 cursor-pointer uppercase tracking-wider shrink-0"
                  title="Manually trigger PDR-Auto workflow"
                >
                  {isPdrAutoRunning ? (
                    <Loader2 className="h-3.5 w-3.5 animate-spin text-yellow-200" />
                  ) : (
            ...
            <div className="flex flex-wrap sm:flex-nowrap gap-3 w-full justify-center items-center sm:items-start">
              ...
              <div className="flex flex-col gap-2 w-[calc(50%-8px)] sm:w-[88px] flex-shrink-0 order-2 sm:order-1">
                ...
                <button
                  ...
                  className="rounded-xl border border-dashed border-slate-700 bg-slate-900/80 hover:bg-indigo-950/40 hover:border-indigo-500/50 transition-all p-2 flex flex-col items-center gap-1 group w-full cursor-pointer"
                >
                ...
                <label
                  title="Upload center poster image from file browser"
                  className="rounded-xl border border-slate-700 bg-slate-900/80 hover:bg-sky-950/40 hover:border-sky-500/50 transition-all p-2 flex flex-col items-center gap-1 group w-full cursor-pointer"
                >
                  <Upload className="h-4 w-4 text-sky-400 group-hover:scale-110 transition-transform" />
                  <span className="text-[8px] font-semibold text-sky-300 text-center leading-tight">Upload Image</span>
                  <span className="text-[7px] text-slate-500 font-mono">Browse</span>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleImageChange}
              ...
              {/* Avatar Character Overlay Layer in Live Simulator (Interactive Drag, Corner Handles & Aspect-Ratio Fixed Crop) */}
              {showAvatar && avatarVideoUrl && (
                <div
                  onMouseDown={(e) => handleMovableMouseDown(e, 'avatar')}
                  onTouchStart={(e) => handleMovableTouchStart(e, 'avatar')}
                  style={{
                    position: 'absolute',
                    left: `${avatarX}%`,
                    top: `${avatarY}%`,
                    width: `${avatarScale}%`,
                    aspectRatio: croppedAvatarAspectRatio ? `${croppedAvatarAspectRatio}` : '9/16',
                    zIndex: avatarLayerPosition === 'behind' ? 15 : 35,
                    cursor: isLockedAvatar ? 'default' : ((isDragging && dragTarget.current === 'avatar') ? 'grabbing' : 'grab'),
                    touchAction: 'none',
                    userSelect: 'none',
                    border: '1.5px dashed transparent',
                    borderColor: activeWatermark === 'avatar' ? '#FF9100' : 'transparent',
                    padding: '2px',
                    boxShadow: activeWatermark === 'avatar' ? '0 0 10px rgba(0,0,0,0.5)' : 'none',
                    transition: (isDraggingAvatar || isResizing) ? 'none' : 'left 0.1s ease, top 0.1s ease, width 0.1s ease, aspect-ratio 0.1s ease'
                  }}
                  className="group/avatar rounded-lg"
                  title="Click to select avatar, drag to position, or use corner handles to resize"
                >
                  <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative', borderRadius: '6px' }}>
                    <video
                      key={avatarVideoUrl}
                      src={avatarVideoUrl}
                      autoPlay
                      ...
                      onLoadedMetadata={(e) => {
                        ...
                        if (v.videoWidth && v.videoHeight) {
                          setAvatarAspectRatio(v.videoWidth / v.videoHeight);
                        }
                      }}
                      style={{
                        position: 'absolute',
                        width: `${100 / Math.max(0.01, scaleAvatarX)}%`,
                        height: `${100 / Math.max(0.01, scaleAvatarY)}%`,
                        left: `-${cropAvatarLeft / Math.max(0.01, scaleAvatarX)}%`,
                        top: `-${cropAvatarTop / Math.max(0.01, scaleAvatarY)}%`,
                        maxWidth: 'none',
                        maxHeight: 'none',
                        objectFit: 'fill',
                        pointerEvents: 'none'
                  ...
                  {activeWatermark === 'avatar' && !isLockedAvatar && (
                    <>
                      <div
                        onMouseDown={(e) => handleResizeStart(e, 'tl', 'avatar')}
                        onTouchStart={(e) => handleResizeTouchStart(e, 'tl', 'avatar')}
                        className="w-3.5 h-3.5 bg-white border-2 border-orange-500 rounded-full absolute -top-1.75 -left-1.75 cursor-nwse-resize shadow-md hover:scale-125 transition-transform z-50"
                      />
                      <div
                        onMouseDown={(e) => handleResizeStart(e, 'tr', 'avatar')}
                        onTouchStart={(e) => handleResizeTouchStart(e, 'tr', 'avatar')}
                        className="w-3.5 h-3.5 bg-white border-2 border-orange-500 rounded-full absolute -top-1.75 -right-1.75 cursor-nesw-resize shadow-md hover:scale-125 transition-transform z-50"
                      />
                      <div
                        onMouseDown={(e) => handleResizeStart(e, 'bl', 'avatar')}
                        onTouchStart={(e) => handleResizeTouchStart(e, 'bl', 'avatar')}
                        className="w-3.5 h-3.5 bg-white border-2 border-orange-500 rounded-full absolute -bottom-1.75 -left-1.75 cursor-nesw-resize shadow-md hover:scale-125 transition-transform z-50"
                      />
                      <div
                        onMouseDown={(e) => handleResizeStart(e, 'br', 'avatar')}
                        onTouchStart={(e) => handleResizeTouchStart(e, 'br', 'avatar')}
                        className="w-3.5 h-3.5 bg-white border-2 border-orange-500 rounded-full absolute -bottom-1.75 -right-1.75 cursor-nwse-resize shadow-md hover:scale-125 transition-transform z-50"
                      />
                    </>
                  )}
                </div>
                  ...
                  switch (style) {
                    case 'blast':
                      return {
                        ...
                        textShadow: '-2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000, 2px 2px 0 #000, 0 3px 0 #000, 3px 0 0 #000, 3px 3px 0 #C2410C, 4px 4px 0 #9A3412',
                        padding: '4px 10px',
                        fontWeight: 900,
...
{/* â”€â”€â”€ Live API Key & Quota Status Panel â”€â”€â”€ */}
      ...
      {/* ────────────────── AVATAR BACKGROUND REMOVER (CHROMA KEY) MODAL ────────────────── */}
      {showChromaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl space-y-5 text-slate-200">
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-100">Avatar Background Remover</h3>
                  <p className="text-[10px] text-slate-400">Apply Chroma Key green screen removal to create clean transparent avatars.</p>
                </div>
              </div>
              <button
                type="button"
            ...
            {/* Error Message */}
            {chromaError && (
              <div className="rounded-xl border border-red-500/30 bg-red-950/20 p-3 text-xs text-red-300 font-medium flex items-center gap-2">
                <AlertCircle className="h-4 w-4 shrink-0 text-red-400" />
                <span>{chromaError}</span>
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              {/* 1. Upload Video */}
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">1. Select Raw Avatar Video (MP4 / WEBM)</label>
                <input
                  type="file"
                  accept="video/*"
                  onChange={(e) => {
                    if (e.target.files && e.target.files[0]) {
                      setChromaVideoFile(e.target.files[0]);
                      setChromaError('');
                    }
                  }}
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 p-2 text-xs text-slate-300 file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-emerald-600 file:text-white hover:file:bg-emerald-500 cursor-pointer"
                />
                {chromaVideoFile && (
                  <p className="text-[10px] text-emerald-400 mt-1 font-mono">
                    Selected: {chromaVideoFile.name} ({(chromaVideoFile.size / (1024 * 1024)).toFixed(2)} MB)
                  </p>
                )}
              </div>

              {/* 2. Target Background Color Picker */}
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1.5">2. Target Background Color to Remove</label>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => { setChromaKeyColor('#00FF00'); setChromaSimilarity(0.35); setChromaBlend(0.10); }}
                    className={`flex-1 py-1.5 px-1.5 rounded-xl text-[10px] font-bold border flex items-center justify-center gap-1 transition-all cursor-pointer ${
                      chromaKeyColor === '#00FF00'
                        ? 'border-emerald-400 bg-emerald-950/60 text-emerald-300 shadow'
                  ...
                  <button
                    type="button"
                    onClick={() => { setChromaKeyColor('#000000'); setChromaSimilarity(0.18); setChromaBlend(0.05); }}
                    className={`flex-1 py-1.5 px-1.5 rounded-xl text-[10px] font-bold border flex items-center justify-center gap-1 transition-all cursor-pointer ${
                      chromaKeyColor === '#000000'
                        ? 'border-slate-400 bg-slate-800 text-white shadow'
                        : 'border-slate-800 bg-slate-950 text-slate-400'
                    }`}
                  >
                    <span className="w-2.5 h-2.5 rounded-full bg-slate-950 border border-slate-600 shrink-0" />
                    Black
                  </button>

                  <button
                    type="button"
                    onClick={() => { setChromaKeyColor('#FFFFFF'); setChromaSimilarity(0.15); setChromaBlend(0.05); }}
                    className={`flex-1 py-1.5 px-1.5 rounded-xl text-[10px] font-bold border flex items-center justify-center gap-1 transition-all cursor-pointer ${
                  ...
                  <div className="flex items-center gap-1.5 bg-slate-950 p-1.5 rounded-xl border border-slate-800">
                    <span className="text-[10px] text-slate-400 font-semibold">Custom Color:</span>
                    <input
                      type="color"
              ...
              {/* White Ring / Border Line Removal Checkbox */}
              <label className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-800 bg-slate-950/60 cursor-pointer">
                <input
                  type="checkbox"
                  checked={removeWhiteRing}
                  onChange={(e) => setRemoveWhiteRing(e.target.checked)}
                  className="w-4 h-4 rounded accent-emerald-500 cursor-pointer"
                />
                <div>
                  <span className="text-xs font-bold text-slate-200">Remove White Circle Arc & Border Lines</span>
                  <p className="text-[9px] text-slate-400">Strips white ring/circle border artifacts behind the avatar's head.</p>
                </div>
              </label>

              {/* 3. Color Similarity Tolerance Slider */}
              <div className="space-y-1">
                <div className="flex justify-between text-xs font-bold">
                  <span className="text-slate-300">Color Similarity Tolerance</span>
                  <span className="text-emerald-400 font-mono">{chromaSimilarity.toFixed(2)}</span>
                </div>
                <input
                  type="range"
                  min="0.05"
                  ...
                  onChange={(e) => setChromaSimilarity(parseFloat(e.target.value))}
                  className="w-full accent-emerald-500 cursor-pointer"
                />
                <p className="text-[9px] text-slate-400">Increase tolerance if background is not fully removed. Decrease if subject edges get cut.</p>
              </div>

              {/* 4. Edge Softness & Despill Slider */}
              <div className="space-y-1">
                <div className="flex justify-between text-xs font-bold">
                  <span className="text-slate-300">Edge Softness / Blend</span>
                  <span className="text-emerald-400 font-mono">{chromaBlend.toFixed(2)}</span>
                </div>
                <input
                  type="range"
                  min="0.01"
                  ...
                  className="w-full accent-emerald-500 cursor-pointer"
                />
                <p className="text-[9px] text-slate-400">Controls edge alpha feathering to prevent green spill on hair and clothing.</p>
              </div>
            ...
            <div className="flex items-center justify-end gap-2 border-t border-slate-800 pt-4">
              ...
              <button
                type="button"
                onClick={handleProcessChromaKey}
                disabled={isProcessingChroma || !chromaVideoFile}
                className="px-5 py-2 rounded-xl text-xs font-bold bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 disabled:opacity-50 text-white shadow-md flex items-center gap-2 transition-all cursor-pointer"
              >
                {isProcessingChroma ? (
                  <>
                  ...
                  <>
                    ...
                    Render & Save Transparent Avatar
                  </>
                )}
              </button>
            </div>
